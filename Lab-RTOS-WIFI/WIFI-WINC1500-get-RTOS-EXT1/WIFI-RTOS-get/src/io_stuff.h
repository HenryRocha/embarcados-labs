...
#define BUT_PLACA_PIO_IDX 11
#define BUT_PLACA_PIO_IDX_MASK (1 << BUT_PLACA_PIO_IDX)
...
void setup_but(short int but, unsigned int pinAttributes, unsigned int edge, void *(*callback)(void *)) {
    switch (but) {
        case 0:
            // BUT PLACA
            pmc_enable_periph_clk(BUT_PLACA_PIO_ID);
            pio_configure(BUT_PLACA_PIO, PIO_INPUT, BUT_PLACA_PIO_IDX_MASK, pinAttributes);
            pio_handler_set(BUT_PLACA_PIO, BUT_PLACA_PIO_ID, BUT_PLACA_PIO_IDX_MASK, edge, callback);
            pio_enable_interrupt(BUT_PLACA_PIO, BUT_PLACA_PIO_IDX_MASK);
...
void switch_led(short int led, short int state) {
    switch (led) {
        case 0:
            if (state) {
                pio_clear(LED_PLACA_PIO, LED_PLACA_IDX_MASK);
