...
SemaphoreHandle_t xSemaphoreAFEC;
...
static void AFEC_pot_Callback(void){
	...
	xSemaphoreGiveFromISR(xSemaphoreAFEC, &xHigherPriorityTaskWoken);
...
void task_adc(void){
	...
	xSemaphoreAFEC = xSemaphoreCreateBinary();
	
	if (xSemaphoreAFEC == NULL) printf("falha em criar o semaforo \n");


	while(1){
		if (xSemaphoreTake(xSemaphoreAFEC, (TickType_t)500) == pdTRUE) {
